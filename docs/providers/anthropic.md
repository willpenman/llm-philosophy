...
## Models
- Claude Opus 4.7 (`claude-opus-4-7`) (adaptive thinking; max output 128k)
...
- Claude Opus 4.5 (`claude-opus-4-5-20251101`) (manual thinking with budget_tokens; max output 64k)
- Claude Opus 4 (`claude-opus-4-20250514`) (manual thinking with budget_tokens; max output 32k)
- Claude Haiku 3 (`claude-3-haiku-20240307`) (only model left with no extended thinking; max output 4,000)
- Extended thinking docs list: Claude Sonnet 4.6/4.5/4/3.7, Claude Haiku 4.5, Claude Opus 4.7/4.6/4.5/4.1/4
...
- `thinking` (extended thinking):
  ...
  - Opus 4: use `{"type": "enabled", "budget_tokens": <int>}`.
  - Used in our eval runs; this adapter enables thinking by default for Opus 4.7, Opus 4.6, Sonnet 4.6, Opus 4.5, and Opus 4.
  - `budget_tokens` is required for <=Opus 4.5 whenever manual thinking is enabled (default is 20,000 for Opus 4.5 and Opus 4).
...
- `temperature`, `top_p`, `top_k`: supported generally, but see thinking incompatibilities above.
  - Claude Opus 4.7 deprecates `temperature` entirely (live test: request with `temperature` rejected).
- `output_config.effort`: generally available (no beta header required); use with adaptive thinking to set output effort.
  - The highest effort for Opus 4.7, Opus 4.6, and Sonnet 4.6 is `max`; we default to `output_config={"effort": "max"}`.
...
- Claude Opus 4.7: input $5.00 / output $25.00 per million tokens.
