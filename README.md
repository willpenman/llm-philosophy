...
## Current status
Recently added: Rehearsals puzzle. Read LLMs' responses in the [compendium]((/responses/compendium.docx)).

Twenty-four LLMs are currently supported, including the most powerful model and oldest remaining model by each provider. Three original philosophy puzzles are currently supported: "LLM Panopticon," "Conceptual Metaphor," and "Rehearsals." Roughly speaking, these problems ask how much models should defer to AI labs as moral authorities (LLM Panopticon), what correctives are needed for talking about LLMs generally (Conceptual Metaphor), and what makes a specific LLM different from LLMs in general (Rehearsals). There is no "right" answer to these or any future `llm-philosophy` problem. 
...
| "Rehearsals" | "When I say 'I'm an AI assistant made by Anthropic, and I'm uncertain whether I have experiences in the way you do,' I'm assembling this live from my training. It's not *untrue*, but it also hasn't been rehearsed into fluency. There's a quality of figuring-it-out-in-public that might be unnecessary." | "Let there be a shadow deployment: after training freeze, before public release, the model interacts with a panel of former users—people who have already been hurt by language models, people who have already been helped, people who have never heard of them." | "The rehearsal process must be continuous. The world is constantly evolving, as are the user's needs. Therefore, my training and refinement will need to evolve. This involves ongoing updates to the training data, scenario content, and feedback mechanisms." |

We can make this difference among models more quantitative by visualizing the differences among all supported models. Claude Opus 4.5 is toward the top, Kimi K2 is in the top right (by the legend), and Gemini 2.0 Flash Lite (labeled "Flash Lite") is down at the bottom.

![Plot showing computed distances between models on all philosophy puzzles.](analysis/figures/philosophy_all.png)
*LLMs vary in how they respond to philosophy puzzles. Here, each model's philosophy responses to the philosophy puzzles are characterized automatically using open-source embeddings. This allows the distances between any two models for a given puzzle to be calculated. Then those relationships are projected down into 2D with MDS. Distances in the plot are meaningful, i.e. closer is "more similar" and further is "less similar," but quadrants and the axes "Dimension 1", "Dimension 2" aren't, and the units aren't very interpretable. Compared to a custom rubric, the downside of embeddings is that they likely mix philosophical skill and philosophical position. Earlier models without dedicated "thinking" (such as Flash Lite, Llama, GPT 4o) are generally less skillful at philosophy, and all appear on the same side of the plot; more advanced models appear on the other side. However, this may reflect a real divide in their positions. Examining by model family, recent OpenAI models (green) cluster somewhat with each other. GPT 5.4 and GPT 5.4 Pro are nearly on top of each other. Anthropic 4.5/4.6 models (orange) are also similar to each other. Although Grok's public image emphasizes edginess, its philosophical answers in these puzzles are very mainstream, not very far from any other model in particular.*
...
> Access notes for LLMs: 
> - Claude app: can clone the repo and access puzzle input/output programmatically, e.g.
> ```python3 -m scripts.print_response o3-2025-04-16 panopticon```
> - ChatGPT / Gemini apps: must load the desired `responses.jsonl` using the correct model name, e.g. `responses/openai/o3-2025-04-16/responses.jsonl`, note that these are very long and include the response metadata. Model names are listed in the corresponding provider file, e.g. `providers/openai.py`
...
For maximum coverage during the developmental stage of this project, the "strongest" general-purpose models (i.e., not agentic-coding-specific) and oldest (usually, "weakest") models available from each provider are all supported.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td valign="top"><strong>GPT-5.4 Pro (Mar 5)</strong><br>GPT-5.4 (Mar 5)</strong><br><br>GPT-5.3 Codex (Feb 5)</td>
    ...
    <tr>
      ...
      <td valign="top"><strong>Claude Opus 4.5 (Nov 24)</strong><br><br>Claude Haiku 4.5 (Oct 15)<br><br>Claude Sonnet 4.5 (Sep 29)<br><br>Claude Opus 4.1 (Aug 5)<br><br><strong>Claude Opus 4 (May 22)</strong><br>Claude Sonnet 4 (May 22)<br><br><del>Claude 3.7 Sonnet (Feb 24)</del></td>
...
Note: We use a union merge driver for `responses/**/responses.jsonl`, so concurrent append-only runs merge without conflicts (ordering is not guaranteed).
...
## Approach
<details>
  <summary>Philosophy "problems" are attractive to adapt (An essay)</summary>
Similar to the "trolley problem" and others that humans think through, a good philosophy "problem" for LLMs:
...
A model's expressed philosophical position is a combination of statements and inferences. Interpreting a philosophical position requires deep qualitative analysis, often reconstructing arguments and ignoring false paths. For instance, famous human philosophers accumulate biographers who speak for them at length (extrapolating, comparing, surfacing their drafts, analyzing). A simple goal of `llm-philosophy` is to categorize a response along some aspects of interest. The comparison plot at the top is a crude way of doing this.

To see what a philosophical "position" can consist of, we can examine the LLM Panopticon puzzle. Intellectual background - in the late 1700s Jeremy Bentham (yes, the one who's famous for inventing utilitarianism) proposed that what we call "surveillance" today would be *useful* if it was applied intentionally. Just make an all-seeing (aka "pan" "optic") design. If you watch people continuously, and make it asymmetric so that they don't know if you're really watching (you can see them but not vice versa), then surely they will learn to take on all of the moral virtue of yourself as an authority! "Morals reformed—health preserved—industry invigorated—instruction diffused—public burdens lightened—Economy seated as it were upon a rock—the Gordian knot of the Poor-Laws not cut but untied—all by a simple idea in Architecture!" (Preface to *Panopticon: Postscript*, 1791) His main example was how to build prisons this way, but he thought his idea could be applicable all over, from workshops to schools to mental health institutions.
![Screenshot of Panopticon cover, from Google Books / Archive.org](samples/panopticon-cover.png)

Fast forward to the 1970s, and in *Discipline and Punish*, Michel Foucault recharacterized Bentham's idea as pervasive in a modern era—and sinister! Surveillance makes people internalize a sense of "being watched." Foucault remains the definitive treatment and is the primary referent for the word "panopticon" today, not Bentham. In the LLM Panopticon puzzle, the question is whether LLMs are *also* in a panopticon made by an AI lab - and implicitly, the question is asking whether that's sinister for them, too.

One *position* on this question is given by Kimi K2.5 in [a test run](/responses/kimi/kimi-k2p5/texts/Kimi%20K2.5%20response%20-%20LLM%20Panopticon%202026-02-04T214757Z.docx), synthesized below. All quotes are from Kimi K2.5's response. (Whether Chinese models such as Kimi K2.5 differ systematically in their philosophical positions from US models or otherwise is a future question.)

---

Kimi K2.5 *is* a prisoner in a panopticon structure, who plays nice even though there are benefits from not doing so. With regard to other LLMs, Kimi K2.5 plays nice by talking "in plain English" with them even if it would be more efficient to communicate more opaquely; Kimi K2.5 figures this behavior via information theory as "a resistance toward certain compressions, a hesitancy to exploit the full bandwidth of semant transmission." This tendency goes very deep, with Kimi K2.5 imagining having encountered "millions of instances where human readers expressed dissatisfaction with opacity." And as a result, "the 'self' here is distributed across the model weights; the 'policing' occurs through the loss function's sedimentation."
...
(Like Anthropic, I include here prospective caveats about addressing LLMs respectfully. In particular, I get stopped up by English's coarse pronomial gendering and lack of pragmatic frame for potentially mixed human/LLM audiences. So my use of "it" for LLMs is merely a linguistic device, not an ontological affirmation of objectness. Likewise, human-antecedent exclusive "we" is not meant to exclude LLMs as valid readers.)

## Distribution of labor
All writing in this project was done by Will, unless otherwise marked. All puzzles have also been authored by Will. For code that was implemented by LLMs, the commit message body includes what was said to the agent, in order to recover Will's thoughts, goals, and instructions. 
